...
# Note: I don't multiply any measures by 100 and don't use ROUND() to set up % data type in power bi and create more accurate graphs.
# ROUND(COUNT(value_1) / COUNT(value_2) * 100, 2) AS measure
...
SELECT COALESCE(utm_source, 'direct') AS utm_source,
    ...
    SUM((price_usd - cogs_usd) * items_purchased) AS profit
...
SELECT QUARTER(s.created_at) AS quarter, MONTHNAME(s.created_at) AS month_name, MONTH(s.created_at) AS month, device_type,
	COUNT(s.website_session_id) AS sessions, COUNT(order_id) AS orders,
    SUM((price_usd - cogs_usd) * items_purchased) AS profit
FROM website_sessions AS s
LEFT JOIN orders AS o 
...
SELECT QUARTER(s.created_at) AS quarter, MONTHNAME(s.created_at) AS month_name, MONTH(s.created_at) AS month, COALESCE(utm_source, 'direct') AS utm_source,
	COUNT(s.website_session_id) AS sessions, COUNT(order_id) AS orders,
    COUNT(order_id) / COUNT(s.website_session_id) AS conversion_rate
FROM website_sessions AS s
LEFT JOIN orders AS o 
...
WITH homepage_test AS (
	SELECT s.website_session_id, s.created_at, order_id, ((price_usd - cogs_usd) * items_purchased) AS profit,
...
# Is there any session that visited two pages during test?
# If yes, it is worth identifying sessions where the entry page = 'home' and 'lander-1', afterwards perform the main calculations.
# In our case, this did not occur, and each distinct session has only one entry page.
...
# In this case, I chose to use ROW_NUMBER windows function instead of use subquery with MIN(created_at) date.
...
WITH billing_test AS (
	SELECT s.website_session_id, order_id, (price_usd * items_purchased) AS revenue, p.created_at AS pageview_date, pageview_url
	FROM website_sessions AS s
	LEFT JOIN orders AS o 
	ON s.website_session_id = o.website_session_id
	INNER JOIN website_pageviews AS p
